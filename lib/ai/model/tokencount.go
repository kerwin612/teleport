...
func (tc *TokenCount) CountAll(ctx context.Context) (int, int) {
	return tc.Prompt.CountAll(ctx), tc.Completion.CountAll(ctx)
}
...
type TokenCounter interface {
	TokenCount(ctx context.Context) int
}
...
func (tc TokenCounters) CountAll(ctx context.Context) int {
	var total int
	for _, counter := range tc {
		total += counter.TokenCount(ctx)
	}
	return total
}
...
func (tc *StaticTokenCounter) TokenCount(_ context.Context) int {
	return int(*tc)
}
...
func (tc *AsynchronousTokenCounter) TokenCount(ctx context.Context) int {
