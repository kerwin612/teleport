...
func (h *AuthHandlers) hostKeyCallback(hostname string, remote net.Addr, key ssh.PublicKey) error {
	...
	// For SubKindOpenSSHEphemeralKeyNode we use SSH Keys (EC2 does not support Certificates in ec2.SendSSHPublicKey).
	if h.c.Server.TargetMetadata().ServerSubKind == types.SubKindOpenSSHEphemeralKeyNode {
		return nil
	}
