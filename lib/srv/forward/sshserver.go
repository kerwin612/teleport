...
func (s *ServerConfig) CheckDefaults() error {
	...
	if s.IsAgentlessNode {
		...
		if s.TargetServer.GetSubKind() == types.SubKindOpenSSHEphemeralKeyNode && s.ProxyPublicAddress == "" {
			return trace.BadParameter("missing parameter ProxyPublicAddress")
		}

		if s.TargetServer.GetSubKind() == types.SubKindOpenSSHEphemeralKeyNode && s.DestinationListener == nil {
...
func (s *Server) Serve() {
	...
	if s.targetServer != nil && s.targetServer.IsOpenSSHNode() {
		...
		if s.targetServer.GetSubKind() == types.SubKindOpenSSHEphemeralKeyNode {
			sshSigner, err := s.setupTunnelForOpenSSHEphemeralNode(ctx)
			if err != nil {
				s.log.Debugf("Unable to set up Tunnel for %q: %v", s.targetServer.GetName(), err)
				return
			}
...
func (s Server) setupTunnelForOpenSSHEphemeralNode(ctx context.Context) (ssh.Signer, error) {
	...
	openTunnelResp, err := awsoidc.OpenTunnelEC2(ctx, openTunnelClt, awsoidc.OpenTunnelEC2Request{
		...
		AvailabilityZoneID: awsInfo.AvailabilityZone,
	...
	// This is the Key that the client must used to connect to the EC2
	// This Key is trusted on the ec2 instance because awsoidc.OpenTunnelEC2 sends the public key to the host.
